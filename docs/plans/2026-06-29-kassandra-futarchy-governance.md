...
### F6 — v0.6 futarchy/Squads execution SEAM + honest deferral (DONE 2026-06-29)

No layout change (no `state.rs` / `state_layout.rs` churn; no new `Ix`/`KassandraError`). F6 was **narrowed to the documented seam fallback** (plan line 72): a prior attempt STALLED trying to hand-build the full Squads-v4 multisig + futarchy v0.6 + Meteora proposal→trade→execute lifecycle in LiteSVM — too heavy. F6 delivers the rigorous seam + honest docs instead. **NO faked "full futarchy passed and executed" result.**

**Squads v4 discriminator scheme — RESOLVED + validated against the real binary.** Squads v4 (`Squads-Protocol/v4` @ rev `6d5235da`, program `SQDS4ep65T869zMMBKyuUq6aD6EgTu8psMjkvj52pCf`) is an **Anchor** program, so instruction selectors are the same `sha256("global:<snake_case_name>")[..8]` scheme as futarchy/Meteora/the vault. The DAO-execution seam is `vault_transaction_execute` → `[0xc2,0x08,0xa1,0x57,0x99,0xa4,0x19,0xab]`. A dispatch-probe against the dumped `squads_v4.so` (the F0 pattern) confirms the real binary dispatches our computed discriminator into its `VaultTransactionExecute` handler (logs `Instruction: VaultTransactionExecute`) and rejects a bogus discriminator. Discriminators added to `metadao_v06.rs` (a Squads section): `SQUADS_VAULT_TRANSACTION_EXECUTE`, `SQUADS_VAULT_TRANSACTION_CREATE`, `SQUADS_PROPOSAL_CREATE`, `SQUADS_MULTISIG_CREATE_V2`. The vault/multisig PDA seed builders (`squads_vault_seeds`/`squads_multisig_seeds` + the `…_pda` derivers) already existed from F0.

**Fixture (committed):** `programs/kassandra/tests/fixtures/squads_v4.so` (1470416 B, sha256 `dec8d3e0…01d34c`) added to the include-set + to `scripts/fetch-metadao-v06.sh` (id, rev/source, sha pin, dump command). The script's existing pinned-hash drift check now covers it too.

**What is driven AGAINST THE REAL BINARIES (not seam-only):**
- All FOUR fixtures (futarchy, conditional_vault v0.6, Meteora DAMM v2, **squads_v4**) load into LiteSVM + are executable (`all_v06_fixtures_load`).
- The Squads `vault_transaction_execute` discriminator + Anchor dispatch scheme, against `squads_v4.so` (`squads_vault_transaction_execute_discriminator_recognized`).
- The Squads vault-PDA seed derivation is self-consistent + stable (`squads_vault_pda_derivation`).
- (Carried from F0: futarchy `initialize_dao` discriminator dispatch; a full conditional_vault `split_tokens` CPI; the futarchy spot-TWAP offset map.)

**Kassandra-side gate — proven END-TO-END AT THE SEAM** (`tests/governance_seam.rs`, 3 tests, all green):
- `set_config_gate_accepts_only_recorded_squads_vault_pda`: DERIVE the Squads v4 vault PDA from the documented seeds (`[b"multisig", b"multisig", dao]`→multisig, `[b"multisig", multisig, b"vault", [0]]`→vault under `SQDS4…`), record it as `Protocol.dao_authority` via `set_governance`, then show `set_config` REJECTS both the admin/payer AND an unrelated keypair with `Unauthorized` (no test signer can be the PDA), and the config is unchanged. Proves the gate accepts ONLY the recorded vault PDA.
- `resolve_deadend_gate_rejects_non_vault_signer`: same, for `resolve_deadend` over a dead-ended oracle — stranger + admin both `Unauthorized`; the oracle stays `InvalidDeadend`.
- `gate_accepts_recorded_authority_when_signable`: accept-path anchor — when the recorded `dao_authority` is an ordinary signable keypair, the SAME gate ACCEPTS it (proving identity-check, not blanket-reject). Full accept/reject matrix already lives in F3/F4; this is a minimal re-anchor.

**HONEST DEFERRAL — what is seam-only / NOT driven in LiteSVM:**
- A test (like any client) CANNOT fabricate a signature for a PDA; only the owning program can `invoke_signed` it. So once `dao_authority` is a Squads vault PDA, NO test keypair satisfies the gate. The production vault-PDA signature on `set_config`/`resolve_deadend` is produced by Squads' `vault_transaction_execute` CPI (shape validated above), NOT driven here.
- The FULL lifecycle — futarchy `initialize_dao` (needs Squads multisig + mints) → `initialize_proposal`/`launch_proposal` → conditional pass/fail KASS markets (v0.6 vault + Meteora DAMM v2) → trade to a pass verdict → `finalize_proposal` → Squads `vault_transaction_execute` of the staged `set_config`/`resolve_deadend` CPI → assert Kassandra state changed — is DEFERRED. Each step is individually heavy (variable-length Borsh params, Meteora liquidity/TWAP cranking over slots, the multisig+proposal+vault-transaction account graph); composing them green in LiteSVM is what stalled the prior attempt.
- **Follow-up for full integration:** run it on a **real validator / surfpool** (not LiteSVM), where the futarchy + Squads + Meteora programs run with their full account graph and the vault PDA signs natively via the genuine `vault_transaction_execute` CPI. The seam proven here (gate accepts only the recorded vault PDA; Squads execute discriminator/dispatch recognized) is exactly the contract that full integration would exercise end-to-end.

Build: `just build` (SBF) clean. Tests: `cargo test -p kassandra-program` all pass (incl. 3 new in `governance_seam.rs` + 2 new in `metadao_v06_cpi.rs`). `cargo clippy --all-targets` clean; `cargo fmt` applied.
