...
- **As of Task 7:** `Ix::FinalizeAiClaims=8`. Errors `DuplicateClaim=12, InvalidOption=13`. New fields (sizes UNCHANGED — absorbed from padding): `Oracle.ai_finalized_count: u16` @194 (bump now @196, `_pad1:[u8;3]`); `Proposer.ai_finalized: u8` @86 (`_pad:[u8;1]`), helper `is_ai_finalized()`. `config::FLIP_SLASH_NUM=1/FLIP_SLASH_DEN=2` (50% flip slash). AiClaim PDA seeds `[b"claim", oracle, proposer]`. `submit_ai_claim` accounts: `[0]oracle(w),[1]proposer(w),[2]ai_claim(w,uninit),[3]authority(signer,w),[4]system`; payload `model_id[32]++params_hash[32]++io_hash[32]++option u8` (exact 97 bytes). `finalize_ai_claims` is **incremental** like finalize_facts (oracle + proposer tail): no-show (`claim_option==NONE`) → FULL slash + disqualify + `surviving_count--`; flipped → PARTIAL slash `bond*1/2` into `bond_pool`, stays surviving; honest → no slash. Marks `ai_finalized`; advances to `Challenge` (window=now+PHASE_WINDOW) when `ai_finalized_count==proposer_count`. No token CPI. Disqualified-before-AiClaim proposers are marked+counted, not re-slashed (defensive).
