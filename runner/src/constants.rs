...
//!
//! These are RE-EXPORTED / DERIVED from the on-chain program crate
//! ([`kassandra_program`]) rather than mirrored, so there is no risk of the
//! runner drifting from the source of truth. The compile-time assertions below
//! pin the `submit_ai_claim` payload layout to the actual [`AiClaim`] account
//! struct: if the program's field order or widths ever change, this crate fails
//! to build.

use core::mem::offset_of;
use kassandra_program::state::AiClaim;

/// `Proposer.claim_option` sentinel: no AI claim submitted yet. Re-exported
/// from the program so the runner and chain agree on the 0xFF "none" value.
pub use kassandra_program::state::CLAIM_OPTION_NONE;
